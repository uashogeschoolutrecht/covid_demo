
---
title: "`r paste('Covid Cases for', params$month, 'in', params$geo_id)`"
author: "Marc A.T. Teunis"
date: "`r Sys.time()`"
output:
  html_document: default
  powerpoint_presentation: default
  word_document: default
  pdf_document: default
params:
  geo_id: NL BE
  month: April May June July August September
  year: 2020
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
  echo = FALSE,
  warning = FALSE,
  message = FALSE,
  error = FALSE
  
  )
```


```{r}
## Packages
library(utils)
library(tidyverse)
library(tools)
```

```{r, echo=TRUE}
## Data
#read the Dataset sheet into “R”. The dataset will be called "data".
#data <- read_csv(
#  "https://opendata.ecdc.europa.eu/covid19/casedistribution/csv", 
#  na = "")
source(
  here::here(
    "data-raw",
    "D010",
    "supporting",
    "covid_ecdc_cases_geography.R"))
```


```{r, include=FALSE}
## Filter for `r params$month` and `r params$geo_id`
## create a conversion table to go from month name to month integer
convert_months <- tibble(
  month_name = c(
    "january",
    "february",
    "march",
    "april",
    "may",
    "june",
    "july",
    "august",
    "september",
    "october",
    "november",
    "december"),
  month_number = c(1:12))
  m_names <- str_split(tolower(params$month), pattern = " ") %>% unlist()
  month_id <- convert_months %>%
    dplyr::filter(
      month_name %in% m_names) %>% 
    dplyr::select(month_number)
  
  id <- month_id$month_number
geo_ids <- str_split(string = params$geo_id, pattern = " ") %>% unlist()
data_filter <- data %>%
  dplyr::filter(
    month %in% id,
     geoId %in% geo_ids
)
  
char_col <- map_lgl(
  .x = data_filter,
  .f = is.numeric 
)
map(
  .x = data_filter[, !char_col],
  .f = unique
)
country_names <- data_filter$countriesAndTerritories %>% unique() %>%
  paste(collapse = ", ")
Caps <- function(x) {
s <- strsplit(x, " ")[[1]]
paste(toupper(substring(s, 1,1)), substring(s, 2),
sep="", collapse=" ")
}
cap_cases <- paste("Cases for", params$month, "and", country_names)
```

## Cases
```{r, fig.cap= cap_cases}
data_filter_agr <- data_filter %>% 
  mutate(date_time = lubridate::dmy(dateRep)) %>%
group_by(week = cut(date_time, "week"), geoId) %>% summarise(value = sum(cases)) 

#names(data_filter)
plot_cases <- data_filter_agr %>%
  ggplot(aes(x = week, y = value)) +
  geom_point(aes(colour = geoId)) +
  geom_line(aes(group = geoId, colour = geoId))
  plot_cases + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```

## Deaths
```{r, fig.cap=paste("Deaths for", params$month, "and" , country_names)}
data_filter_deaths_agr <- data_filter %>% 
  mutate(date_time = lubridate::dmy(dateRep))  %>%
group_by(week = cut(date_time, "week"), geoId) %>% 
  summarise(value = sum(deaths)) 

#names(data_filter)
plot <- data_filter_deaths_agr %>%
  ggplot(aes(x = week, y = value)) +
  geom_point(aes(colour = geoId)) +
  geom_line(aes(group = geoId, colour = geoId)) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  
plot
```

